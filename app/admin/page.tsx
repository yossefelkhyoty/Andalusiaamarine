/**
 * ANDALUSIA MARINE - ADMIN LIVE V11.5 (FULL DB DIAGNOSTIC)
 * FINAL INTEGRATION WITH VERBOSE SERVER ERRORS
 */
...
export default function Admin() {
  ...
  const deleteProject = async (id: number) => {
    ...
    if (res.ok) {
        ...
        alert('Delete Operation Aborted: ' + (err.details || err.error))
    }
    setIsSyncing(false)
  }
  ...
  const saveToDb = async () => {
    ...
    if (res.ok) {
        ...
        const err = await res.json()
        // 🛡 SHOW REAL DETAILED ERROR (schema/type mismatches)
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans italic">
      <aside className="w-72 bg-slate-950 text-white p-10 flex flex-col fixed h-screen z-50 transition-all italic shadow-[10px_0_30px_rgba(0,0,0,0.1)]">
      ...
      <main className="flex-grow pl-[20rem] p-16 italic">
         ...
         {isSyncing && <div className="fixed top-10 right-10 bg-slate-950 text-white px-8 py-4 rounded-full z-[100] shadow-2xl border border-amber-600/30 flex items-center gap-5 animate-bounce"><Loader2 className="w-5 h-5 text-amber-500 animate-spin" /><span className="text-[10px] font-black uppercase tracking-widest">Global Live Syncing...</span></div>}
